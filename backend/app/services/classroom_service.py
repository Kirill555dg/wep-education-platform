...
class ClassroomService:
    ...
    def get_student_classrooms(
        ...
        return [ClassroomResponse.model_validate(c) for c in classrooms if c]

    def update_classroom(
        self, classroom_id: int, classroom_data: ClassroomUpdate, teacher_user_id: int
    ) -> ClassroomResponse:
        ...
        if not teacher or classroom.teacher_id != teacher.id:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only classroom owner can update it")
    ...
    def join_classroom(self, join_data: JoinClassroomRequest, student_user_id: int) -> ClassroomResponse:
        ...
        if existing and existing.is_active:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Already a member of this classroom")
        ...
        if existing:
            ...
            self.student_classroom_repo.create({"student_id": student.id, "classroom_id": classroom.id})
