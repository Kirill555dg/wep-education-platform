...
class UserRepository(BaseRepository[User]):
    ...
    def get_by_username_or_email(self, username_or_email: str) -> tp.Optional[User]:
        ...
        return (
            self.db.query(User).filter((User.username == username_or_email) | (User.email == username_or_email)).first()
        )

    def get_with_login_data(self, user_id: int) -> tp.Optional[User]:
        """Get user with login data"""
        return self.db.query(User).options(joinedload(User.login_data)).filter(User.id == user_id).first()

    def get_with_profile(self, user_id: int) -> tp.Optional[User]:
        """Get user with teacher/student profile"""
        return (
            self.db.query(User)
            .options(joinedload(User.teacher), joinedload(User.student))
...
class TeacherRepository(BaseRepository[Teacher]):
    ...
    def get_with_user(self, teacher_id: int) -> tp.Optional[Teacher]:
        ...
        return self.db.query(Teacher).options(joinedload(Teacher.user)).filter(Teacher.id == teacher_id).first()

    def get_by_subject(self, subject: str, skip: int = 0, limit: int = 100) -> tp.List[Teacher]:
        """Get teachers by subject specialization"""
        return self.db.query(Teacher).filter(Teacher.subject_specialization == subject).offset(skip).limit(limit).all()
...
class StudentRepository(BaseRepository[Student]):
    ...
    def get_with_user(self, student_id: int) -> tp.Optional[Student]:
        ...
        return self.db.query(Student).options(joinedload(Student.user)).filter(Student.id == student_id).first()

    def get_by_grade_level(self, grade_level: int, skip: int = 0, limit: int = 100) -> tp.List[Student]:
        """Get students by grade level"""
        return self.db.query(Student).filter(Student.grade_level == grade_level).offset(skip).limit(limit).all()
