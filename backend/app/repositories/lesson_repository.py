...
from app.models.theory import TheoryMaterial, Subject, Section, Subsection
...
class LessonRepository(BaseRepository[Lesson]):
    ...
    def get_by_classroom(self, classroom_id: int, skip: int = 0, limit: int = 100) -> tp.List[Lesson]:
        ...
        ).order_by(Lesson.order_number).offset(skip).limit(limit).all()
    ...
    def get_with_materials(self, lesson_id: int) -> tp.Optional[Lesson]:
        ...
        return self.db.query(Lesson).options(
            joinedload(Lesson.lesson_materials)
...
class LessonMaterialRepository(BaseRepository[LessonMaterial]):
    ...
    def get_by_lesson(self, lesson_id: int) -> tp.List[LessonMaterial]:
        ...
        return self.db.query(LessonMaterial).filter(
            LessonMaterial.lesson_id == lesson_id
        ).order_by(LessonMaterial.order_number).all()
    
    def add_material_to_lesson(self, lesson_id: int, material_id: int, order_number: int = 0, is_required: bool = True) -> LessonMaterial:
...
class TheoryMaterialRepository(BaseRepository[TheoryMaterial]):
    ...
    def get_by_subsection(self, subsection_id: int, skip: int = 0, limit: int = 100) -> tp.List[TheoryMaterial]:
        ...
        return self.db.query(TheoryMaterial).filter(
            TheoryMaterial.subsection_id == subsection_id
        ).order_by(TheoryMaterial.order_number).offset(skip).limit(limit).all()
    ...
    def get_published(self, subsection_id: int, skip: int = 0, limit: int = 100) -> tp.List[TheoryMaterial]:
        ...
        return self.db.query(TheoryMaterial).filter(
            TheoryMaterial.subsection_id == subsection_id,
            TheoryMaterial.is_published == True
        ).order_by(TheoryMaterial.order_number).offset(skip).limit(limit).all()
...
class SubjectRepository(BaseRepository[Subject]):
    ...
    def get_active(self, skip: int = 0, limit: int = 100) -> tp.List[Subject]:
        ...
        ).order_by(Subject.order_number).offset(skip).limit(limit).all()
